...
static void update_save_path(const char* ndspath,const char* gbapath){
	...
	if(preloaded_nds_data){
		...
		save_path_nds = savebase + ndsname+".sav";
	}
	else if(!ndspath){
	...
	if(preloaded_gba_data){
		...
		save_path_gba = savebase + gbaname+".srm";
	}
	else if(!gbapath){
...
static bool _handle_load_game(unsigned type, const struct retro_game_info *info)
{
   ...
   */
   ...
   struct retro_input_descriptor desc[] = {
      ...
      { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_G8, "Swap screens" },
      { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_MENU, "Close lid" },
